...
// into the assessment.
// v5 (July 2026) removed the study paths, going straight into assessment
// after province.
...
async function advance(s, input, rawBody) {
  ...
  if (s.step === "province") {
    ...
    s.step = "status";
    return statusPiece(s, lang);
  }

  if (s.step === "status") {
    const statusOptions = ["Grade 12", "Unemployed", "Studying", "Working"];
    const statusIndex = parseInt(input, 10) - 1;
    if (isNaN(statusIndex) || statusIndex < 0 || statusIndex >= statusOptions.length) {
      return [{ type: "list", text: i18n.t(lang, "askStatusRetry"), button: i18n.t(lang, "statusListButton"), rows: statusOptions }];
    }
    s.data.status = statusOptions[statusIndex];
    s.step = "funding";
    return fundingPiece(s, lang);
  }

  if (s.step === "funding") {
    const fundingOptions = ["Bursary / NSFAS", "Student Loan", "Family/Self-Funded", "Unsure"];
    const fundingIndex = parseInt(input, 10) - 1;
    if (isNaN(fundingIndex) || fundingIndex < 0 || fundingIndex >= fundingOptions.length) {
      return [{ type: "list", text: i18n.t(lang, "askFundingRetry"), button: i18n.t(lang, "fundingListButton"), rows: fundingOptions }];
    }
    s.data.funding = fundingOptions[fundingIndex];
    s.step = "assessment";
    return startAssessment(s, lang);
  }
...
// Status piece for employment/education status
function statusPiece(s, lang) {
  return {
    type: "list",
    text: i18n.t(lang, "askStatus"),
    button: i18n.t(lang, "statusListButton"),
    rows: ["Grade 12", "Unemployed", "Studying", "Working"],
  };
}

// Funding piece for tuition funding plans
function fundingPiece(s, lang) {
  return {
    type: "list",
    text: i18n.t(lang, "askFunding"),
    button: i18n.t(lang, "fundingListButton"),
    rows: ["Bursary / NSFAS", "Student Loan", "Family/Self-Funded", "Unsure"],
  };
}

// ---- Study-path pieces (v4) ----

// After onboarding, go straight to assessment (no study paths).
function pathChoicePiece(s) {
  const lang = langOf(s);
  return { type: "menu", text: i18n.t(lang, "pathChoicePrompt", s.data.name), ids: ["assessment"], buttons: i18n.t(lang, "pathButtons") };
}
