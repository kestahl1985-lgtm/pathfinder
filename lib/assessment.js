...
const study = require("./study_paths.js");
...
// v4 (July 2026) inserted a 3-way path choice after province (assessment /
// "I know what to study" / "study by subject") instead of going straight
// into the assessment.
...
const VALID_STEPS = new Set([
  ...
  "path_choice", "study_category", "study_list", "subjects_maths",
...
async function advance(s, input, rawBody) {
  ...
  if (s.step === "province") {
    ...
    // v4: profile is complete — offer the 3-way path choice rather than
    // dropping straight into the assessment.
    s.step = "path_choice";
    return [pathChoicePiece(s)];
  }

  // The 3-way menu shown right after onboarding. Buttons carry fixed ids
  // ("assessment" / "know" / "subjects"); a learner who types instead of
  // tapping is matched leniently below.
  if (s.step === "path_choice") {
    const v = String(input || "").trim().toLowerCase();
    if (v === "assessment" || v === "quiz" || v.includes("quiz")) return startAssessment(s, lang);
    if (v === "subjects" || v.includes("subject")) {
      s.step = "subjects_maths";
      return [mathsPiece(s)];
    }
    if (v === "know" || v.includes("know") || v.includes("study")) {
      s.step = "study_category";
      return [studyCategoryPiece(s)];
    }
    // Unrecognised — re-show the menu.
    return [pathChoicePiece(s)];
  }

  // "I already know what I want to study" — pick an interest area (its id is
  // a RIASEC letter), then a career within it.
  if (s.step === "study_category") {
    const v = String(input || "").trim().toUpperCase();
    if (v === "QUIZ") return startAssessment(s, lang);
    if (study.CATEGORY_ORDER.includes(v)) {
      s.data.study_list = study.careersInCategory(v);
      s.data.study_area = v;
      s.step = "study_list";
      return [{ type: "text", text: careerListText(s, lang) }];
    }
    return [studyCategoryPiece(s)];
  }

  // A numbered list of careers (from either the category pick or the
  // subjects filter) is on screen; the learner taps a number to see what it
  // takes to study it, or navigates with AREAS / QUIZ.
  if (s.step === "study_list") {
    const v = String(input || "").trim().toLowerCase();
    if (v === "areas" || v === "area") { s.step = "study_category"; return [studyCategoryPiece(s)]; }
    if (v === "quiz" || v === "assessment") return startAssessment(s, lang);
    if (v === "subjects" || v.includes("subject")) { s.step = "subjects_maths"; return [mathsPiece(s)]; }
    const list = s.data.study_list || [];
    const n = parseInt(input, 10);
    if (n >= 1 && n <= list.length) {
      const careerId = list[n - 1];
      const pieces = [{ type: "text", text: study.requirementText(lang, careerId) + i18n.t(lang, "studyNav") }];
      // Surface a matching sponsor course, same as the assessment path does.
      const c = careerById(careerId);
      const sponsor = await findSponsorMatch(s.data.province, c.traits);
      if (sponsor) {
        pieces[0].text =
          study.requirementText(lang, careerId) +
          `${i18n.t(lang, "sponsoredNear")}\n*${sponsor.college.name}* — ${sponsor.course.name}` +
          (sponsor.college.website ? `\n${sponsor.college.website}` : "") +
          i18n.t(lang, "sponsorInfoPrompt") +
          i18n.t(lang, "studyNav");
        s.data.last_sponsor = {
          college_name: sponsor.college.name, course_name: sponsor.course.name,
        ...
        if (sponsor.course.prospectus_url) pieces.push(prospectusPiece(lang, sponsor));
      }
      return pieces;
    }
    return [{ type: "text", text: i18n.t(lang, "studyPickInvalid") + "\n\n" + careerListText(s, lang) }];
  }

  // "What could I study from my subjects?" — v1 filters by the Maths a
  // learner takes, the single biggest NSC gate. Buttons carry ids
  // "pure" / "lit" / "none".
  if (s.step === "subjects_maths") {
    const v = String(input || "").trim().toLowerCase();
    if (v === "quiz" || v === "assessment") return startAssessment(s, lang);
    let have = null;
    if (v === "pure" || v.includes("pure")) have = "pure";
    else if (v === "lit" || v.includes("liter") || v.includes("geletter")) have = "lit";
    else if (v === "none" || v.includes("no ") || v.includes("geen") || v.includes("awa") || v.includes("aku")) have = "none";
    if (!have) return [mathsPiece(s)];
    const ids = study.reachableByMaths(have);
    if (!ids.length) return [{ type: "text", text: i18n.t(lang, "reachableNone") }];
    s.data.study_list = ids;
    s.data.study_area = null;
    s.step = "study_list";
    return [{ type: "text", text: i18n.t(lang, "reachableHeader", ids.length) + careerListText(s, lang, true) }];
  }
...
// ---- Study-path pieces (v4) ----

// The 3-way menu after onboarding. Fixed button ids assessment/know/subjects.
function pathChoicePiece(s) {
  const lang = langOf(s);
  return { type: "menu", text: i18n.t(lang, "pathChoicePrompt", s.data.name), ids: ["assessment", "know", "subjects"], buttons: i18n.t(lang, "pathButtons") };
}

// "Which Maths do you take?" — fixed button ids pure/lit/none.
function mathsPiece(s) {
  const lang = langOf(s);
  return { type: "menu", text: i18n.t(lang, "subjectsMathsPrompt"), ids: ["pure", "lit", "none"], buttons: i18n.t(lang, "mathsButtons") };
}

// Interest-area list picker; row ids are RIASEC letters.
function studyCategoryPiece(s) {
  const lang = langOf(s);
  return { type: "list", text: i18n.t(lang, "studyCategoryPrompt"), button: i18n.t(lang, "studyCategoryButton"), rows: study.categoryRows(lang) };
}

// A numbered, tappable-by-number list of the careers currently in
// s.data.study_list. suppressHeader skips the category heading (the subjects
// path prepends its own "N careers open to you" header instead).
function careerListText(s, lang, suppressHeader) {
  const list = s.data.study_list || [];
  let text = "";
  if (!suppressHeader && s.data.study_area) text += i18n.t(lang, "studyCareerListHeader", study.categoryTitle(lang, s.data.study_area));
  list.forEach((id, i) => { text += `*${i + 1}.* ${careerById(id).name}\n`; });
  text += i18n.t(lang, "studyPickInstruction", list.length);
  text += i18n.t(lang, "studyNav");
  return text;
}

// Starts the 30-question assessment: zero the counter AND recorded answers
// (the assessment branch derives its position from responses.length), then
// send the intro + question 1 as ONE piece so WhatsApp can't deliver the
// first question ahead of the intro. Shared by the path menu and any
// direct "QUIZ" reply.
function startAssessment(s, lang) {
  s.step = "assessment";
  s.q = 0;
  s.responses = [];
  return [{ type: "yesno", text: `${i18n.t(lang, "assessmentIntro", s.data.name)}\n\n${questionText(0, lang)}`, buttons: i18n.t(lang, "yesnoButtons") }];
}
...
async function logSponsorMatch(s, sponsor, careerId) {
  ...
  const r = await supabaseRequest("POST", "sponsor_matches", {
    phone: s.phone,
    college_id: sponsor.college.id,
    course_id: sponsor.course.id,
    career_id: careerId,
    // sponsor.score is now a cosine (0–1) from affinity(); sponsor_matches
    // .match_score is an INT column, so store it as a 0–100 percentage. Without
    // this rounding the insert 400s ("invalid input syntax for type integer")
    // and every impression is silently dropped — which would leave the Delivery
    // dashboard empty and false-trigger the clause 3.5 make-good.
    match_score: Math.round((sponsor.score || 0) * 100),
  ...
  // Impression logging feeds the sponsor delivery/floor tracking (clause 3.5).
  // A failed insert must never break the learner's reply — but it must NOT be
  // silent either: a silent match_score type error is exactly how sponsor
  // impressions were being dropped undetected. Surface it in the function logs
  // so a logging outage is visible instead of quietly under-counting delivery.
  if (!r.ok) {
    console.error(
      `[sponsor_matches] impression NOT logged for course ${sponsor.course.id} ` +
      `(HTTP ${r.status || "?"}): ${r.error || (r.json && r.json.message) || "unknown"}`
    );
  }
}
