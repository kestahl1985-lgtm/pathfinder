...
// History: v1 asked grade + suburb; v2 (July 2026) removed grade and asks
// province instead of suburb. Bump this whenever the sequence of steps or
// questions changes.
const FLOW_VERSION = 2;

// Every step the current state machine knows how to handle. A stored
// session pointing at any other step (e.g. "grade"/"suburb" from flow v1,
// or a corrupted value) must NOT be resumed — before this guard existed,
// such sessions fell through to a dead-end "reply RESTART" fallback, and a
// v1 session mid-assessment could surface a question number out of nowhere.
const VALID_STEPS = new Set([
  "language", "consent", "name", "surname", "school", "age", "province",
...
async function advance(s, input, rawBody) {
  ...
  if (s.step === "assessment") {
    ...
    return [
      ...
      reportPiece(s),
      { type: "text", text: i18n.t(lang, "shareInstructions") },
